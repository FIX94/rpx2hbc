...
static void loadFunctionPointers(private_data_t * private_data)
{
    ...
    OS_FIND_EXPORT(sysapp_handle, "SYSLaunchMenu",SYSLaunchMenu);
	OS_FIND_EXPORT(sysapp_handle, "_SYSGetSystemApplicationTitleId",_SYSGetSystemApplicationTitleId);

	EXPORT_DECL(u64, OSGetTitleID, void);
    OS_FIND_EXPORT(coreinit_handle, "OSGetTitleID",OSGetTitleID);
	...
	void (*KPADInit)();
    unsigned int wpad_handle;
    OSDynLoad_Acquire("padscore.rpl", &wpad_handle);
    OSDynLoad_FindExport(wpad_handle, 0, "KPADInit", &KPADInit);

	//needed for CMPT to work
	KPADInit();

	databuf = private_data->MEMAllocFromDefaultHeapEx(datasize,0x40);
	CMPTLaunchTitle(databuf, datasize, 0x00010001, 0x554E454F);
	private_data->MEMFreeToDefaultHeap(databuf);
}
